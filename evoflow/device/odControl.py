...
class ODControl:
    ...
    def __init__(self, V0: float, A0: float, mu0: float, kp: float, ki: float, q_max: float, Ts: float, A_setpoint: float):
        ...
        self.anti_windup_limit = 5     # Limit for integral term to prevent windup
        self.back_calculation_gain = 1  # Back-calculation gain for anti-windup
    ...
    def calculate_dilution_flow(self, current_od: float) -> float:
        ...
        # Compute error
        error = self.A_setpoint - current_od

        # --- Back-calculation anti-windup ---
        # 1) Compute the unsaturated control action first
        q_unsaturated = self.V0 * (self.mu_hat + (self.kp * error) + (self.ki * self.integral))

        # 2) Saturate the flow to the physical actuator limits.
        q_in = max(0.0, min(q_unsaturated, self.q_max))
        ...
        actuator_mismatch = q_in - q_unsaturated
        self.integral += error * self.Ts + self.back_calculation_gain * actuator_mismatch
    ...
    def calculate_outlet_flows(self, preferred_q_lagoon: float) -> tuple[float, float]:
        ...
        if preferred_q_lagoon > self.previous_flow:
            q_lagoon = max(0, min(preferred_q_lagoon, self.previous_flow))
